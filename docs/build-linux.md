...
The Linux CI workflow uses one GitHub-hosted Ubuntu 24.04 x86-64 runner to build
and validate CPython 3.11, 3.12, and 3.13 wheels sequentially. This avoids
reinstalling the native toolchain and Qt SDK for each Python version. Every
`linux_x86_64` wheel receives the same content, RUNPATH, isolated-install, ELF,
and Xvfb checks. ARM64 remains supported and locally validated on Ubuntu 24.04,
but automated ARM64 wheel CI is deferred. The workflow uses `auditwheel show`
for diagnostics only: it does not run `auditwheel repair` or bundle the system
web engine.
